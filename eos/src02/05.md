...
- contracts —— 智能合约目录，一些辅助的ABI的源码
- library/chain —— 区块链目录，是一些编译的接口
- library/wasm-jit —— 是主要的文件部分
- externals/src —— 一部分的二进制编译代码
- 其它一些目录下也有相关的一些文件，但比较分散代码也很少。重点分析编译过程。

虚拟机的模块分成两部分，也就是**编译部分**和**执行部分**。

智能合约在编译过程中会产生两个文件，一个是**.wast**文件,一个是**.abi**文件。

## 2. 编译过程

### 2.1 wast文件的生成
eosiocpp是编译智能合约的命令，在tools目录下，eosiocpp.in中：
